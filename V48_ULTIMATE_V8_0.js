...
const SBA_V47_ULTIMATE = (function() {
    ...
    function getContextWeights(sharpSignal = null, modelDirection = null, isComplex = false) {
        const w = {};
        // V8.0: Load adaptive weights from tracked results (if available)
        let adaptive = {};
        try { adaptive = getAdaptiveEngineWeights(); } catch(e) {}
        const hasAdaptive = Object.keys(adaptive).length >= 5; // Need 5+ engines tracked
        ...
        for (const [eng, cfg] of Object.entries(ENGINE_TIERS)) {
            ...
            // V8.0: ADAPTIVE BLEND — if we have tracked results, blend them in (30% adaptive + 70% static)
            if (hasAdaptive && adaptive[eng]) {
                weight = weight * 0.7 + (adaptive[eng] * cfg.base) * 0.3;
            }
            w[eng] = weight;
        }
        // Normalize to 1.0
    ...
    function crossValidateStats(bdlStats, espnStats, sport, player, statKey = 'pts') {
        ...
        if (hasBDL && hasESPN) {
            ...
            const mergedStats = {
                ...
                position: bdlStats.position || espnStats.position || null,
...
reasoning must explain your analysis`;
    ...
    async function analyze(params) {
        ...
        if (isPlayerProp && params.player && params.opponent) {
            ...
            if (h2hData.found) {
                ...
                console.log(`[V5.4] ${h2hIcon} H2H vs ${params.opponent}: ${h2hData.avgVs} avg (${h2hData.trend === 'SIMILAR' ? 'similar to usual' : h2hData.trend + ' than usual'})`);
            }
        }
        ...
        let oddsAPIRaw = null;
        try {
            oddsAPIRaw = await getOddsAPIPlayerProps(sport, params.player, market, params.line, null);
        } catch(e) { /* oddsAPIRaw optional */ }
        if (oddsData.found) {
            ...
            try {
                ...
                if (sport === 'nba' && bdlPlayerId) {
                    ...
                    const oppTeamId = await (async () => {
                        ...
                        try {
                            ...
                            for (const team of allTeams) {
                                // V8.0 FIX: Exact abbreviation match FIRST (prevents "ORL" matching "New Orleans")
                            ...
                            // Fallback: full name / city match (only if abbreviation didn't match)
                            for (const team of allTeams) {
                                if (team.full_name?.toLowerCase().includes(oppName) ||
                                    (oppName.length > 3 && (team.city?.toLowerCase().includes(oppName) ||
                    ...
                    const [matrixRes, advancedRes, shootingRes, clutchRes, oppProfileRes, quartersRes] = 
                        ...
                        // 6. V30 Quarter Breakdowns — computed after parallel fetch (needs clutch data)
                        (async () => { return null; })()
                    ]);
                    ...
                    v7Quarters = null; // Computed below after clutch data available
                    
                    // ── V30 Quarter Profile (derived from clutch + game flow) ──
                    try {
                        if (rawGames && rawGames.length >= 5) {
                            const qSorted = [...rawGames].filter(g => parseMinutes(g.min||'0') > 12)
                                .sort((a,b) => new Date(b.game?.date||b.date) - new Date(a.game?.date||a.date)).slice(0, 15);
                            if (qSorted.length >= 5) {
                                const qStatMap = getStatMapping(sport, statKey, market);
                                let closeGameVals = [], blowoutVals = [];
                                qSorted.forEach(g => {
                                    const val = extractStatValue(g, qStatMap, sport) || 0;
                                    const hs = g.game?.home_team_score || 0, vs = g.game?.visitor_team_score || 0;
                                    const margin = Math.abs(hs - vs);
                                    if (margin <= 10) closeGameVals.push(val);
                                    else blowoutVals.push(val);
                                });
                                
                                // Clutch data as 4Q proxy (now available from parallel fetch)
                                const clutchPts = v7Clutch?.stats?.pts || v7Clutch?.stats?.points || 0;
                                const clutchMin = v7Clutch?.stats?.min || v7Clutch?.stats?.minutes || 0;
                                const clutchRate = clutchMin > 0 ? clutchPts / clutchMin : 0;
                                
                                const avgVal = qSorted.reduce((s,g) => s + (extractStatValue(g, qStatMap, sport)||0), 0) / qSorted.length;
                                const avgMin = qSorted.reduce((s,g) => s + parseMinutes(g.min||'0'), 0) / qSorted.length;
                                const perMinRate = avgMin > 0 ? avgVal / avgMin : 0;
                                
                                const estHalfMins = avgMin / 2;
                                const est1H = perMinRate * estHalfMins;
                                const clutchBoost = clutchRate > perMinRate * 1.1 ? 1.08 : clutchRate < perMinRate * 0.8 ? 0.92 : 1.0;
                                const blowoutPenalty = blowoutVals.length > closeGameVals.length * 0.5 ? 0.95 : 1.0;
                                const est2H = perMinRate * estHalfMins * clutchBoost * blowoutPenalty;
                                
                                const halfDiff = est1H > 0 ? ((est2H - est1H) / est1H * 100).toFixed(1) : '0';
                                const profile = est2H > est1H * 1.05 ? '2H HEAVY' : est1H > est2H * 1.05 ? '1H HEAVY' : 'BALANCED';
                                
                                v7Quarters = { est1H, est2H, halfDiff: parseFloat(halfDiff), profile, clutchRate, perMinRate, closeGames: closeGameVals.length, blowoutGames: blowoutVals.length };
                                console.log(`[V8.0] 📊 Quarter Profile: ${profile} | Est 1H: ${est1H.toFixed(1)} / 2H: ${est2H.toFixed(1)} (${halfDiff}% diff) | Close=${closeGameVals.length}g, Blowout=${blowoutVals.length}g`);
                            }
                        }
                    } catch(e) { /* quarter analysis optional */ }
                    ...
                    // ── V11 Blowout Risk ──
                    const brSpread = oddsData?.spread || minutesData?.spreadEstimate || null;
                    if (brSpread) {
                        const spread = parseFloat(brSpread);
                    ...
                    if (Array.isArray(oddsAPIRaw) && oddsAPIRaw.length > 0) {
                        ...
                        oddsAPIRaw.forEach(o => {
                            ...
                            bookPairs[o.book].point = o.point;
                        });
                        let overBooks = 0, underBooks = 0;
                        Object.values(bookPairs).forEach(bp => {
                            if (bp.over && bp.under) {
                                const overJuice = Math.abs(bp.over);
                                const underJuice = Math.abs(bp.under);
                    ...
                    if (Array.isArray(oddsAPIRaw) && oddsAPIRaw.length > 0) {
                        ...
                        // Look for Pinnacle or sharp books
                        const sharpBooks = ['Pinnacle', 'Circa', 'Bet365', 'BetOnline.ag'];
                        let sharpFound = null;
                        for (const sb of sharpBooks) {
                            const match = Object.entries(pinnBookPairs).find(([name]) => name.toLowerCase().includes(sb.toLowerCase()));
                            if (match && match[1].over && match[1].under) { sharpFound = { name: match[0], ...match[1] }; break; }
                        }
                        if (sharpFound) {
                            const juiceDiff = Math.abs(sharpFound.over) - Math.abs(sharpFound.under);
                            const sharpLean = juiceDiff > 5 ? 'UNDER' : juiceDiff < -5 ? 'OVER' : 'NEUTRAL';
                            // Steam move: sharp book disagrees with market avg
                            const allOvers = Object.values(pinnBookPairs).filter(b => b.over).map(b => Math.abs(b.over));
                            const avgOverJuice = allOvers.reduce((s,v) => s+v, 0) / allOvers.length;
                            const steamMove = Math.abs(Math.abs(sharpFound.over) - avgOverJuice) > 8;
                            v7SharpMoney = v7SharpMoney || {};
                            v7SharpMoney.pinnacle = { book: sharpFound.name, over: sharpFound.over, under: sharpFound.under, juiceDiff, sharpLean, steamMove };
                            console.log(`[V8.0] 🦈 ${sharpFound.name}: Over ${sharpFound.over} / Under ${sharpFound.under} → ${sharpLean}${steamMove ? ' ⚡STEAM MOVE' : ''}`);
                        }
                    }

                    // ── V8.0 Game Script Adjustment ──
                    v7GameScript = null;
                    const gsSpread = minutesData?.spreadEstimate || oddsData?.spread || null;
                    if (v48TeamIntel && gsSpread) {
                        const spread = parseFloat(gsSpread);
                    ...
                    if (sport === 'nba' || sport === 'nhl') {
                        // Playoff Race (teams between .450 and .650 are fighting for seeding)
                        if (v48StandingsData) {
                            const wins = v48StandingsData.wins || 0, losses = v48StandingsData.losses || 0;
                            const pctg = wins / (wins + losses || 1);
                            if (pctg >= 0.450 && pctg <= 0.650) v7Spots.push({ name: 'Playoff Race', adj: 0.4, note: `${wins}-${losses} record (.${(pctg*1000).toFixed(0)}), fighting for seeding` });
                            if (pctg > 0.650) v7Spots.push({ name: 'Elite Team', adj: 0.2, note: `${wins}-${losses} — top contender motivation` });
                        }
                        // Division rivalry
                        const spotOpp = params.opponent || '';
                        if (spotOpp) {
                            ...
                            const oppDiv = allDivs[spotOpp.toUpperCase()];
                            if (teamDiv && oppDiv && teamDiv === oppDiv) v7Spots.push({ name: 'Division Rivalry', adj: 0.3, note: 'Same division matchup' });
                            // Conference rivalry (same conference but diff division)
                            const teamConf = eastDivs[stats.team?.toUpperCase()] ? 'EAST' : westDivs[stats.team?.toUpperCase()] ? 'WEST' : null;
                            const oppConf = eastDivs[spotOpp.toUpperCase()] ? 'EAST' : westDivs[spotOpp.toUpperCase()] ? 'WEST' : null;
                            if (teamConf && oppConf && teamConf === oppConf && teamDiv !== oppDiv) v7Spots.push({ name: 'Conference Game', adj: 0.15, note: `${teamConf} conference matchup` });
                        }
                        // Home game boost
                        const gameInfo = stats.rawGames?.[0]?.game;
                        if (gameInfo && stats.bdlTeamId && gameInfo.home_team_id === stats.bdlTeamId) {
                            v7Spots.push({ name: 'Home Game', adj: 0.2, note: 'Playing at home' });
                        }
                        // Back-to-back detection
                        if (b2bData?.isB2B) v7Spots.push({ name: 'Back-to-Back', adj: -0.5, note: 'B2B fatigue risk' });
                    }
                    if (v7Spots.length > 0) {
                    ...
                    if (Array.isArray(oddsAPIRaw) && oddsAPIRaw.length > 0) {
                        ...
                        oddsAPIRaw.forEach(o => {
                            ...
                            if (o.name === 'Over') bookPairs[o.book].over = o.price;
                            if (o.name === 'Under') bookPairs[o.book].under = o.price;
                            bookPairs[o.book].line = o.point;
                        });
                        const pairs = Object.values(bookPairs).filter(b => b.over && b.under);
                        if (pairs.length > 0) {
                            // Find consensus line, best over, best under
                            const lineCounts = {};
                            pairs.forEach(p => { lineCounts[p.line] = (lineCounts[p.line]||0) + 1; });
                            const consensusLine = Object.entries(lineCounts).sort((a,b) => b[1]-a[1])[0]?.[0];
                            const bestOver = pairs.sort((a,b) => b.over - a.over)[0];
                            const bestUnder = [...pairs].sort((a,b) => b.under - a.under)[0];
                            const avgOver = pairs.reduce((s,p) => s + p.over, 0) / pairs.length;
                            const avgUnder = pairs.reduce((s,p) => s + p.under, 0) / pairs.length;
                            v7PlayerProps = {
                                count: pairs.length,
                                consensusLine: parseFloat(consensusLine),
                                bestOver: { book: bestOver.book, price: bestOver.over },
                                bestUnder: { book: bestUnder.book, price: bestUnder.under },
                                avgOver: Math.round(avgOver),
                                avgUnder: Math.round(avgUnder),
                                altLines: Object.keys(lineCounts).length > 1 ? Object.keys(lineCounts).map(Number).sort((a,b) => a-b) : null
                            };
                            console.log(`[V8.0] 📋 Prop Lines: ${pairs.length} books @ ${consensusLine} | Best Over: ${bestOver.over > 0 ? '+' : ''}${bestOver.over} (${bestOver.book}) | Best Under: ${bestUnder.under > 0 ? '+' : ''}${bestUnder.under} (${bestUnder.book})`);
                            if (v7PlayerProps.altLines && v7PlayerProps.altLines.length > 1) {
                                console.log(`[V8.0]    └─ Alt lines available: ${v7PlayerProps.altLines.join(', ')}`);
                            }
                        }
                    }

                    console.log(`[V7.0] ✅ Full enrichment complete: Matrix=${!!v7Matrix}, Advanced=${!!v7Advanced}, Shooting=${!!v7Shooting}, Clutch=${!!v7Clutch}, OppProfile=${!!v7OppProfile}, Quarters=${!!v7Quarters}, Sharp=${!!v7SharpMoney}, GameScript=${!!v7GameScript}, Props=${!!v7PlayerProps}, Spots=${v7Spots.length}`);
                }
        ...
        if (v48Projection) {
            ...
            if (v7Quarters) aiParams._v7Quarters = `${v7Quarters.profile}: 1H=${v7Quarters.est1H?.toFixed(1)}, 2H=${v7Quarters.est2H?.toFixed(1)} (${v7Quarters.halfDiff}% diff), Close=${v7Quarters.closeGames}g, Blowout=${v7Quarters.blowoutGames}g`;
        ...
        if (isPlayerProp && master && aiCollective) {
            ...
            if (v48Projection?.cv > 35) underFactors.push(`High variance (CV=${v48Projection.cv.toFixed(1)}%)`);
        ...
        if (isPlayerProp && master && bothSides) {
            ...
            // Run 3 fast engines with explicit OVER framing and UNDER framing
            const synthEngines = ['groq', 'together', 'deepseek'];
            ...
            try {
                ...
                let overStrength = 0, underStrength = 0, overCount = 0, underCount = 0;
                synthResults.forEach(sr => {
                    const conf = sr.result?.confidence || 50;
                    if (sr.side === 'OVER') { overStrength += conf; overCount++; }
                    else { underStrength += conf; underCount++; }
                    const icon = sr.result?.pick === 'OVER' ? '🟢' : sr.result?.pick === 'UNDER' ? '🔴' : '⚪';
                    console.log(`[V8.0] ${icon} ${sr.engine.padEnd(10)} [${sr.side}]: ${sr.result?.pick || 'PASS'} @ ${conf}%`);
                });
                
                const avgOverStr = overCount > 0 ? overStrength / overCount : 50;
                const avgUnderStr = underCount > 0 ? underStrength / underCount : 50;
                const synthDirection = avgOverStr > avgUnderStr + 5 ? 'OVER' : avgUnderStr > avgOverStr + 5 ? 'UNDER' : 'SPLIT';
                const synthConfidence = Math.abs(avgOverStr - avgUnderStr);
                
                synthResult = {
                    overStrength: Math.round(avgOverStr),
                    underStrength: Math.round(avgUnderStr),
                    direction: synthDirection,
                    confidence: Math.round(synthConfidence),
                    totalEngines: 11 + synthEngines.length * 2 // 11 original + 6 synthesis = 17 total evaluations
                };
                
                console.log(`[V8.0] 🧪 Synthesis: ${synthDirection} | Over=${Math.round(avgOverStr)}% vs Under=${Math.round(avgUnderStr)}% | ${synthResult.totalEngines} total evaluations`);
                
                // Update conviction with synthesis data
                if (convictionScore && synthConfidence > 10) {
                    const synthBonus = Math.min(10, Math.round(synthConfidence / 3));
                    if (synthDirection === master.finalPick) {
                        convictionScore.sba = Math.min(100, convictionScore.sba + synthBonus);
                        console.log(`[V8.0]    └─ Conviction boost +${synthBonus} → SBA=${convictionScore.sba}/100`);
                    }
                }
                
                // Update bothSides with synthesis
                if (bothSides) {
                    bothSides.synthOver = Math.round(avgOverStr);
                    bothSides.synthUnder = Math.round(avgUnderStr);
                }
            } catch(e) {
                console.log(`[V8.0] ⚠️ Synthesis error: ${e.message}`);
            }
        }
        ...
        if (Array.isArray(oddsAPIData) && oddsAPIData.length > 0 && isPlayerProp) {
            // Group by line, then by book to find over/under pairs
            const lineGroups = {};
            oddsAPIData.forEach(o => {
                const pt = o.point || parseFloat(params.line);
                if (!lineGroups[pt]) lineGroups[pt] = { overs: [], unders: [] };
                if (o.name === 'Over') lineGroups[pt].overs.push({ book: o.book, price: o.price });
                if (o.name === 'Under') lineGroups[pt].unders.push({ book: o.book, price: o.price });
            });
            let arbFound = false;
            ...
            if (!arbFound) console.log(`[V8.0] 🔍 Prop Arb Scan: No arb found across ${Object.keys(lineGroups).length} line(s)`);
        }
        ...
        if (coach.injuryOverride || (lineupData.available && lineupData.isOut)) {
            ...
            if (synthResult) {
                console.log(`║  🧪 SYNTHESIS: ${synthResult.direction} | Over=${synthResult.overStrength}% vs Under=${synthResult.underStrength}% (${synthResult.totalEngines} evals)`.padEnd(78) + '║');
            }
        ...
        const finalResult = {
            ...
            // V8.0: CLV + Engine Tracking + Calibration
            impliedProbAtBet: coach.impliedProb || null,
    ...
    async function getRefereeTendencies(sport, home, away) {
        const refPrompt = `Who is the lead referee/crew chief assigned to tonight's ${sport.toUpperCase()} game: ${home} vs ${away}? Provide their historical tendencies. Reply EXACTLY: REFEREE: [name] TOTALS_BIAS: [OVER/UNDER/NEUTRAL] FOUL_RATE: [HIGH/MEDIUM/LOW] AVG_TOTAL_DIFF: [+X.X or -X.X] CAREER_GAMES: [number] NOTES: [one line] CONFIDENCE: [HIGH/MEDIUM/LOW]`;
        const sources = ['grok', 'perplexity'];
        for (const src of sources) {
            try {
                const res = await fetch(`${PROXY}/api/ai/${src}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: refPrompt, maxTokens: 200 }) });
                if (res.ok) {
                    const data = await res.json();
                    const text = extractText(data);
                    ...
                    if (refMatch && refMatch[1].trim().length > 3) {
                        ...
                        if (/unknown|not available|not assigned|unavailable|cannot/i.test(name)) continue;
                        ...
                        return { found: true, source: `LIVE-${src.toUpperCase()}`, referee: name, totalsBias, foulRate: foulMatch ? foulMatch[1].toUpperCase() : 'MEDIUM', avgTotalDiff: diffMatch ? parseFloat(diffMatch[1]) : 0, careerGames: gamesMatch ? parseInt(gamesMatch[1]) : 0, notes: notesMatch ? notesMatch[1].trim() : '', confidence: confMatch ? confMatch[1].toUpperCase() : 'LOW', adjustment: totalsBias === 'OVER' ? 0.02 : totalsBias === 'UNDER' ? -0.02 : 0 };
                    }
                }
            } catch(e) { console.log(`[V8.0] Referee ${src} error: ${e.message}`); }
        }
...
SOURCE: [where you found this]`,
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // V8.0: CALIBRATION REPORT + CLV TRACKING
    // ═══════════════════════════════════════════════════════════════════════════
    function getCalibrationReport() {
        const stored = JSON.parse(localStorage.getItem('SBA_BET_HISTORY') || '[]');
        const settled = stored.filter(b => b.result && b.result !== null);
        
        console.log('\n╔═══════════════════════════════════════════════════════════════════════════════╗');
        console.log('║       📊 V8.0 CALIBRATION REPORT                                             ║');
        console.log('╠═══════════════════════════════════════════════════════════════════════════════╣');
        
        if (settled.length < 5) {
            console.log(`║  ⚠️ Only ${settled.length} settled bets. Need 5+ for calibration.`.padEnd(78) + '║');
            console.log('║  Use: SBA_V47_ULTIMATE.recordResult("BET_ID", "WIN/LOSS", actualValue)'.padEnd(78) + '║');
            console.log('╚═══════════════════════════════════════════════════════════════════════════════╝');
            return { totalBets: stored.length, settled: settled.length, needMore: true };
        }
        
        // By verdict tier
        const tiers = { 'STRONG BET': [], 'MODERATE BET': [], 'SMALL BET': [], 'NO BET': [] };
        settled.forEach(b => {
            const tier = b.verdict || 'NO BET';
            Object.keys(tiers).forEach(t => { if (tier.includes(t.replace('BET','').trim()) || tier === t) tiers[t].push(b); });
        });
        
        console.log('║  BY VERDICT TIER:'.padEnd(78) + '║');
        for (const [tier, bets] of Object.entries(tiers)) {
            if (bets.length === 0) continue;
            const wins = bets.filter(b => b.result === 'WIN').length;
            const rate = ((wins / bets.length) * 100).toFixed(1);
            const bar = '█'.repeat(Math.floor(wins / bets.length * 20));
            console.log(`║  ${tier.padEnd(14)}: ${wins}/${bets.length} (${rate}%) ${bar}`.padEnd(78) + '║');
        }
        
        // CLV tracking
        const withCLV = settled.filter(b => b.closingLine && b.openLine);
        if (withCLV.length > 0) {
            const clvWins = withCLV.filter(b => {
                const openEdge = Math.abs(parseFloat(b.openLine) - parseFloat(b.line));
                const closeEdge = Math.abs(parseFloat(b.closingLine) - parseFloat(b.line));
                return b.pick === 'OVER' ? parseFloat(b.closingLine) > parseFloat(b.openLine) : parseFloat(b.closingLine) < parseFloat(b.openLine);
            }).length;
            const clvRate = ((clvWins / withCLV.length) * 100).toFixed(1);
            console.log('║'.padEnd(78) + '║');
            console.log(`║  CLV (Beat Closing Line): ${clvWins}/${withCLV.length} (${clvRate}%)`.padEnd(78) + '║');
        }
        
        // Overall
        const totalWins = settled.filter(b => b.result === 'WIN').length;
        const overallRate = ((totalWins / settled.length) * 100).toFixed(1);
        const roi = settled.reduce((sum, b) => {
            const units = parseFloat(b.units) || 0;
            return sum + (b.result === 'WIN' ? units * 0.9 : -units); // -110 juice
        }, 0);
        
        console.log('║'.padEnd(78) + '║');
        console.log(`║  OVERALL: ${totalWins}/${settled.length} (${overallRate}%) | ROI: ${roi >= 0 ? '+' : ''}${roi.toFixed(1)} units`.padEnd(78) + '║');
        console.log('╚═══════════════════════════════════════════════════════════════════════════════╝');
        
        return { totalBets: stored.length, settled: settled.length, winRate: overallRate, roi, tiers };
    }
    return {
        analyze,
        scan,
