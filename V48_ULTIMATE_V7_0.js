            ...
            try {
                ...
                if (['nba','wnba','ncaab'].includes(sport) && bdlPlayerId) {
                    try {
                        ...
                        if (ar.ok) { const ad = (await ar.json()).data?.[0]; if (ad) { advancedStats = { usagePct: ad.usg_pct||ad.usage||null, trueShooting: ad.ts_pct||null }; if (advancedStats.usagePct>0) console.log(`[V48] ✅ Advanced: Usage=${(advancedStats.usagePct*100).toFixed(1)}%`); } }
                    } catch(e) {}
                ...
                if (hasBDL && bdlTeamId) {
                    try {
                        ...
                        if (sr.ok) { const ts = (await sr.json()).data?.find(s=>s.team?.id===bdlTeamId); if (ts) console.log(`[V48] ✅ Standings [${sport.toUpperCase()}]: ${ts.wins}-${ts.losses}`); }
                    } catch(e) {}
                }
                ...
                if (sport === 'nba' && bdlPlayerId) {
                    ...
                    const [matrixRes, advancedRes, shootingRes, clutchRes, oppProfileRes, quartersRes] = 
                        ...
                        (async () => {
                            const categories = [
                                { cat: 'usage', type: null, key: 'usage' },
                                { cat: 'scoring', type: null, key: 'scoring' },
                                { cat: 'defense', type: null, key: 'defense' },
                                { cat: 'misc', type: null, key: 'misc' },
                        ...
                        (async () => {
                            try {
                                ...
                                if (!res.ok) return null;
                                const json = await res.json();
                                const adv = json?.data?.[0];
                                if (adv) console.log(`[V7.0] ✅ V29 Advanced: USG=${(adv.usg_pct*100||0).toFixed(1)}%, TS=${(adv.ts_pct*100||0).toFixed(1)}%`);
                        ...
                        (async () => {
                            try {
                                const fetches = ['less_than_10_ft', 'midrange', 'above_the_break_3', 'overall'].map(async range => {
                                ...
                                const mid = shooting.midrange;
                                const three = shooting.above_the_break_3;
                                if (overall) {
                                    const totalFGA = overall.fga || 1;
                                    shooting.pctFromPaint = (rim?.fga || 0) / totalFGA;
                                    shooting.pctFromMidrange = (mid?.fga || 0) / totalFGA;
                                    shooting.pctFrom3 = (three?.fga || 0) / totalFGA;
                        ...
                        // 6. V30 Quarter Breakdowns
                        (async () => {
                            try {
                                const quarters = {};
                                const qTypes = [
                                    { type: 'q1', label: 'Q1' }, { type: 'q2', label: 'Q2' },
                                    { type: 'q3', label: 'Q3' }, { type: 'q4', label: 'Q4' }
                                ];
                                const results = await Promise.allSettled(qTypes.map(async ({type, label}) => {
                                    const url = `${baseUrl}/nba/v1/season_averages/general?season=${yr}&season_type=regular&type=${type}&player_ids[]=${bdlPlayerId}`;
                                    const res = await fetch(url, { headers: hdrs });
                                    if (!res.ok) return { label, data: null };
                                    const json = await res.json();
                                    return { label, data: json?.data?.[0] };
                                }));
                                results.forEach(r => { if (r.status === 'fulfilled' && r.value?.data) quarters[r.value.label] = r.value.data; });
                                if (Object.keys(quarters).length >= 4) {
                                    const h1 = quarters.Q1 && quarters.Q2 ? { pts: (quarters.Q1.pts||0)+(quarters.Q2.pts||0), usg_pct: ((quarters.Q1.usg_pct||0)+(quarters.Q2.usg_pct||0))/2, efg_pct: ((quarters.Q1.efg_pct||0)+(quarters.Q2.efg_pct||0))/2 } : null;
                                    const h2 = quarters.Q3 && quarters.Q4 ? { pts: (quarters.Q3.pts||0)+(quarters.Q4.pts||0), usg_pct: ((quarters.Q3.usg_pct||0)+(quarters.Q4.usg_pct||0))/2, efg_pct: ((quarters.Q3.efg_pct||0)+(quarters.Q4.efg_pct||0))/2 } : null;
                                    if (h1 && h2) {
                                        quarters.usageDiff = ((h1.usg_pct - h2.usg_pct) * 100).toFixed(1);
                                        quarters.efgDiff = ((h1.efg_pct - h2.efg_pct) * 100).toFixed(1);
                                        console.log(`[V7.0] ✅ Quarter Breakdown: 4/4 | 1H vs 2H: Usage ${quarters.usageDiff > 0 ? '+' : ''}${quarters.usageDiff}%, EFG ${quarters.efgDiff > 0 ? '+' : ''}${quarters.efgDiff}%`);
                                    }
                                }
                                return Object.keys(quarters).length > 0 ? quarters : null;
                            } catch(e) { return null; }
                        })()
                    ]);
