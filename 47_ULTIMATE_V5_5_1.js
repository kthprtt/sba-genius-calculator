// ═══════════════════════════════════════════════════════════════════════════════
// SBA GENIUS V47 ULTIMATE V5.5.1 - SMART ODDS + COACH K NARRATIVE + FEEL LIKE ODDS
// ═══════════════════════════════════════════════════════════════════════════════
    ...
    async function getDefenseRankings(sport) {
        ...
        const parseDefenseArray = (raw) => {
            ...
            try {
                ...
                return Object.keys(rankings).length >= 10 ? rankings : null;
            } catch(e) { return null; }
        };
        ...
        console.log(`[V5.5] ⚠️ All 3 live defense sources failed — using STATIC fallback`);
    ...
    async function getPaceFactor(team, opponent, sport = 'nba') {
        ...
        try {
            const res = await fetch(`${PROXY}/api/ai/grok`, {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    prompt: `${sport.toUpperCase()} pace analysis for ${team} vs ${opponent}:

Reply EXACTLY:
TEAM_PACE_RANK: [1-30 ranking, 1=fastest]
OPP_PACE_RANK: [1-30 ranking]
TEAM_PPG: [points per game]
OPP_PPG_ALLOWED: [points allowed per game]
PACE_IMPACT: [FAST/SLOW/NEUTRAL]
SCORING_PROJECTION: [HIGH/LOW/AVERAGE]`,
                    maxTokens: 120
                })
            });
...
MINUTES: [expected minutes]`,
    ...
    async function getBDLStatsUnified(player, sport, statKey = 'pts', market = 'player_points') {
        ...
        try {
            ...
            const statsUrl = `https://api.balldontlie.io/${endpoint}/v1/stats?player_ids[]=${p.id}&seasons[]=${seasonYear}&per_page=100`;
            ...
            // Extract stat values and filter low-minute games
            const gameValues = [];
            for (const g of games) {
                // Filter low-minute games for basketball
                if (['nba', 'wnba', 'ncaab'].includes(sport)) {
...
LAST_5: [num1, num2, num3, num4, num5]`,
    ...
    async function callEngine(engineId, params, stats, hitRate, sgoData, defenseData, sport, market) {
        ...
        if (isPlayerProp) {
            // PLAYER PROP PROMPT
            const statsText = stats ? `L5=${getStatVal(stats.l5, statKey)} L10=${getStatVal(stats.l10, statKey)} Season=${getStatVal(stats.season, statKey)}. Recent games: ${hitRate?.values?.join(', ') || 'N/A'}` : 'Stats unavailable';
...
reasoning must explain your analysis`;
    ...
    async function analyze(params) {
        ...
        console.log('\n');
        console.log('╔═══════════════════════════════════════════════════════════════════════════════╗');
        console.log('║       🏆 SBA GENIUS V47 ULTIMATE V5.5.1 - MAXIMUM ACCURACY 🏆             ║');
        console.log('╠═══════════════════════════════════════════════════════════════════════════════╣');
        ...
        await Promise.all(engines.map(async (engineId) => {
            try {
                const result = await callEngine(engineId, params, stats, hitRate, oddsData, defenseData, sport, market);
